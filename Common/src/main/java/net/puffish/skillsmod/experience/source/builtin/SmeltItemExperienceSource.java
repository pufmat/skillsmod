...
public record SmeltItemExperienceSource(
		Calculation<Data> calculation
) implements ExperienceSource {

	private static final Identifier ID = SkillsMod.createIdentifier("smelt_item");
	...
	static {
		...
		PROTOTYPE.registerOperation(
				SkillsMod.createIdentifier("get_smelted_item_stack"),
	...
	public static void register() {
		SkillsAPI.registerExperienceSource(
				ID,
				SmeltItemExperienceSource::parse
		);
	}

	private static Result<SmeltItemExperienceSource, Problem> parse(ExperienceSourceConfigContext context) {
		return context.getData()
				.andThen(JsonElement::getAsObject)
				.andThen(rootObject -> rootObject.noUnused(o -> parse(o, context)));
	}

	private static Result<SmeltItemExperienceSource, Problem> parse(JsonObject rootObject, ExperienceSourceConfigContext context) {
		...
		if (problems.isEmpty()) {
			return Result.success(new SmeltItemExperienceSource(
