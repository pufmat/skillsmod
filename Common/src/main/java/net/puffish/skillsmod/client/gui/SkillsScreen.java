	...
	private void drawTabs(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		forEachVisibleTab((x, category) -> {
			if (category.hasUnseenPoints()) {
				RenderSystem.setShaderTexture(0, TRIAL_ICON_TEXTURE);
				DrawableHelper.drawTexture(
						matrices,
						x + 10,
						FRAME_PADDING - 5,
	...
	private void drawWindowWithCategory(MatrixStack matrices, double mouseX, double mouseY, ClientCategoryData activeCategoryData) {
		...
		if (activeCategoryData.hasExperience()) {
			...
			if (activeCategoryData.getCurrentLevel() > 0) {
				tmpText = new LiteralText("" + activeCategoryData.getCurrentLevel());
		...
		if (activeCategoryData.hasExchange()) {
			...
			if (belowLimit) {
				tmpText = new LiteralText(String.valueOf(activeCategoryData.getCurrentCost()));
				tmpX += 103;
				tmpX -= this.textRenderer.getWidth(tmpText);
