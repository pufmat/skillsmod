...
public class SkillsScreen extends Screen {
	...
	private static final Identifier ENCHANTMENT_SLOT_DISABLED_TEXTURE = Identifier.withDefaultNamespace("container/enchanting_table/enchantment_slot_disabled");
	...
	private void drawWindowWithCategory(GuiGraphicsExtractor graphics, int mouseX, int mouseY, ClientCategoryData activeCategoryData) {
		...
		if (activeCategoryData.hasExchange()) {
			...
			var canAfford = minecraft.player.experienceLevel >= activeCategoryData.getCurrentCost();
			...
			if (insideBuyButton) {
				...
				if (belowLimit) {
					lines.add(SkillsMod.createTranslatable(
							"tooltip",
							"cost",
							minecraft.player.experienceLevel + "/" + activeCategoryData.getCurrentCost()
					).getVisualOrderText());
				}
				graphics.setTooltipForNextFrame(lines, mouseX, mouseY);
			}
			...
			if (belowLimit) {
				tmpText = Component.literal(String.valueOf(activeCategoryData.getCurrentCost()));
				tmpX += 103;
				tmpX -= this.font.width(tmpText);
